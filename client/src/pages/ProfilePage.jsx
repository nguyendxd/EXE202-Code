...
export default function ProfilePage() {
    ...
    useEffect(() => {
        ...
        const fetchPets = async () => {
            ...
            setPetsError(null);
            try {
                const res = await getPetsByUserId(id || currentUser._id);
    ...
    // Khi mở modal cập nhật, điền sẵn thông tin pet
    useEffect(() => {
        if (editingPet) {
            ...
            setPetForm({
                ...
                healthStatus: Array.isArray(editingPet.healthStatus) ? editingPet.healthStatus.join(', ') : (editingPet.healthStatus || ''),
                color: editingPet.color || '',
                weight: editingPet.weight || '',
                temperament: editingPet.temperament || '',
                address: editingPet.address || '',
                contactPhone: editingPet.contactPhone || '',
        ...
        <div style={{
                    ...
                    <div style={{
                        ...
                        <form onSubmit={async (e) => {
                            ...
                            try {
                                ...
                                const res = await getPetsByUserId(user._id);
                                setPets(res.data.data || []);
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 18 }}>
                                    <button type="button" onClick={() => { setShowPetModal(false); setEditingPet(null); }} style={{ padding: '10px 28px', borderRadius: 8, border: '2px solid #E5C299', background: 'transparent', color: '#5C4033', fontWeight: 600, cursor: 'pointer', fontSize: 15 }}>Huỷ</button>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                    <div style={{ fontWeight: 700, fontSize: 22, color: '#5C4033' }}>Các động vật đã đăng</div>
                    <button onClick={() => setShowPetModal(true)} style={{ background: '#E5C299', color: '#5C4033', border: 'none', borderRadius: '20px', padding: '10px 24px', fontSize: 16, fontWeight: 600, cursor: 'pointer', boxShadow: '0 2px 8px rgba(229,194,153,0.12)' }}>Đăng động vật</button>
                </div>
                {petsLoading ? (
